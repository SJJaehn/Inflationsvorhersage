function stats=regstats2(responses,data,model,whichstats,L)
...
...         %------------------------------------------------------
            'hc0','hc1','hc2','hc3','hc4','hac','qlike','empty','rankdef','boot'};
            %------------------------------------------------------
...
%----------------------------------------------------------------  
  ...
  if idx(27) % HC3
   ...
   if idx(33) % Bootstrapped SEs
        stats.(varnames{33}).beta = beta;
        stats.(varnames{33}).t = fBootstrapPvals(responses, data);     
        stats.(varnames{33}).pval =  2*(tcdf(-abs(stats.(varnames{33}).t), dfe));
        stats.(varnames{33}).df = dfe;
   end
