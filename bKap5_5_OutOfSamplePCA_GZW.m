...
% (out-of-sample). At each origin a PCA + regression (PCR) is re-estimated on a
...
% Initialize memory
vYhat   = NaN(iNumObs,1);     % PCR forecast
...
for iIdxT = iNumIn:iNumObs
    ...
    % Estimate PCA on the in-sample predictors and project the out-of-sample
    % row using the IN-SAMPLE mean/std and eigenvectors (no look-ahead).
    rModel     = fEstPCA(mXin, "iNumComp", iNumComp, "iTransformX", iTransformX);
    mScoresIn  = rModel.mScores;
    mScoresOut = fProjectData(vXout, rModel.mEigVec, rModel.vMeanX, rModel.vStdX);

    % PCR: regress inflation on the in-sample scores (with constant)
    mBetaPCR     = [ones(size(mXin,1), 1), mScoresIn] \ vYin;
    vYhat(iIdxT) = [1, mScoresOut] * mBetaPCR;
...
fprintf('One-step-ahead OOS inflation forecast (PCR)\n');
fprintf('  PCs = %d | window = %s (min in-sample %d) | forecasts = %d\n\n', ...
