...
At each origin the candidate sets are scored on the in-sample window:
  - holdout metrics (RMSE/R2OOS/...): k-fold time-series cross-validation
    (sklearn TimeSeriesSplit) over the window; out-of-fold predictions are
    pooled and the metric computed once. This respects time order (each fold
    trains on the past, tests on the future), so there is no look-ahead.
  - information criteria (AIC/BIC): fit on the FULL window, score in-sample
    (these penalise complexity directly, so no CV is used).
...
def cv_score(y, X, cols, win):
    """Pool out-of-fold predictions across TimeSeriesSplit folds over `win`,
    then compute METRIC once. Used for the holdout metrics."""
    tscv = TimeSeriesSplit(n_splits=N_SPLITS)
    yt, yh, yb = [], [], []
    for tr, te in tscv.split(win):
        fit_idx = win[tr]
        test_idx = win[te]
        yh.append(predict_set(y, X, cols, fit_idx, test_idx))
        yt.append(y[test_idx])
        yb.append(np.full(len(test_idx), y[fit_idx].mean()))   # per-fold benchmark
    ytrue = np.concatenate(yt)
    yhat = np.concatenate(yh)
    ybm = np.concatenate(yb)
...
def score_set(y, X, cols, win):
    ...
    if INFO_CRIT:
        # fit on the full window, score in-sample by AIC/BIC (no CV)
        Xc = X[:, cols] if len(cols) else None
        return util.info_criterion(y, Xc, win, which=METRIC)
    return cv_score(y, X, cols, win)
...
print(f"Selection metric: {METRIC} "
      ...
      f"{'in-sample/full window' if INFO_CRIT else f'{N_SPLITS}-fold TimeSeriesSplit CV'})")
...
#  Save
# =========================================================================
options = f"train{TRAIN_OBS}_{util.window_tag(ROLLING)}_{METRIC}_splits{N_SPLITS}"
out_dir = util.result_dir(OUTPUT_DIR, "selection_cv", COUNTRY, "oos", options)
